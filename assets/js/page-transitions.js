...
(function () {
  function normalizePathname(p) {
    var prefixes = (window.MEOWODY_LANG_PREFIXES || []).join('|');
  ...
  window.addEventListener('pageshow', function (e) {
    if (e.persisted) {
      document.documentElement.removeAttribute('data-page-enter');
      sessionStorage.removeItem('meowody_page_animation');
    }
  });
  ...
  function initPageTransitions() {
    var allMenuLinks = document.querySelectorAll('.header-menu .menu-link, .mobile-menu-nav .menu-link, .tag-nav-link, .logo-link');
    allMenuLinks.forEach(function (link) {
      link.addEventListener('click', function (e) {
        ...
        var dir = goingForward ? 'swipe-left' : 'swipe-right';
        sessionStorage.setItem('meowody_page_animation', dir);
      });
    });
  }
